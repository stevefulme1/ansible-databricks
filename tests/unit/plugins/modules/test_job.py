"""Unit tests for stevefulme1.databricks.job module."""
...
MODULE_PATH = "ansible_collections.stevefulme1.databricks.plugins.modules.job"
CLIENT_PATH = "ansible_collections.stevefulme1.databricks.plugins.module_utils.databricks_client"


@pytest.fixture
def mock_api_client():
    """Mock API client for job."""
    client = MagicMock()
    client.get.return_value = None
    client.create.return_value = {"job_id": "res-123", "name": "test-job"}
    client.update.return_value = {"job_id": "res-123", "name": "test-job-updated"}
...
def existing_resource():
    """Return a dict representing an existing job."""
    return {
        "job_id": "res-123",
        "name": "test-job",
        "state": "active",
...
class TestCreateJob:
    ...
    def test_create_returns_resource(self, mock_api_client):
        ...
        result = mock_api_client.create("job", {"name": "test-job"})
    ...
    def test_create_with_all_params(self, mock_api_client):
        ...
        mock_api_client.create("job", params)
        mock_api_client.create.assert_called_once_with("job", params)
...
class TestUpdateJob:
    ...
    def test_update_existing_resource(self, mock_api_client, existing_resource):
        ...
        result = mock_api_client.update("job", "res-123", {"name": "updated"})
        assert result["name"] == "test-job-updated"
    ...
    def test_update_nonexistent_raises(self, mock_api_client):
        ...
        with pytest.raises(Exception, match="404 Not Found"):
            mock_api_client.update("job", "bad-id", {})


class TestDeleteJob:
    """Tests for deleting a job."""

    def test_delete_existing(self, mock_api_client, existing_resource):
        """Verify delete calls API with correct ID."""
        mock_api_client.get.return_value = existing_resource
        mock_api_client.delete("job", "res-123")
        mock_api_client.delete.assert_called_once_with("job", "res-123")

    def test_delete_nonexistent_is_noop(self, mock_api_client):
        """Verify deleting absent resource reports no change."""
        mock_api_client.get.return_value = None
        result = mock_api_client.get("job", "missing-id")
...
class TestGetJob:
    ...
    def test_get_api_timeout(self):
        ...
        with pytest.raises(TimeoutError):
            client.get("job", "res-123")


class TestListJob:
    """Tests for listing job resources."""

    def test_list_returns_all(self, mock_api_client):
        """Verify list returns all resources."""
        mock_api_client.list.return_value = [
            {"job_id": "1", "name": "first"},
            {"job_id": "2", "name": "second"},
        ]
        result = mock_api_client.list("job")
        assert len(result) == 2
    ...
    def test_list_with_filter(self, mock_api_client):
        ...
        mock_api_client.list.return_value = [{"job_id": "1", "name": "match"}]
        result = mock_api_client.list("job", filters={"name": "match"})
        assert len(result) == 1


class TestIdempotencyJob:
    """Tests for idempotent behavior of job."""
    ...
    def test_create_existing_is_idempotent(self, mock_api_client, existing_resource):
        ...
        mock_api_client.get.return_value = existing_resource
        current = mock_api_client.get("job", "res-123")
    ...
    def test_delete_absent_is_idempotent(self, mock_api_client):
        ...
        mock_api_client.get.return_value = None
        exists = mock_api_client.get("job", "missing") is not None
        assert exists is False


class TestErrorHandlingJob:
    """Tests for error handling in job."""
