"""Unit tests for stevefulme1.databricks.cluster module."""

from unittest.mock import MagicMock

import pytest

MODULE_PATH = "ansible_collections.stevefulme1.databricks.plugins.modules.cluster"
...
def mock_api_client():
    ...
    client.create.return_value = {"cluster_id": "res-123", "cluster_name": "test-cluster"}
    client.update.return_value = {"cluster_id": "res-123", "cluster_name": "test-cluster-updated"}
...
class TestCreateCluster:
    ...
    def test_create_returns_resource(self, mock_api_client):
        ...
        result = mock_api_client.create("cluster", {"cluster_name": "test-cluster"})
    ...
    def test_create_api_error(self):
        ...
        with pytest.raises(Exception, match="409 Conflict"):
            client.create("cluster", {"cluster_name": "dup"})
...
class TestUpdateCluster:
    ...
    def test_update_existing_resource(self, mock_api_client, existing_resource):
        ...
        result = mock_api_client.update("cluster", "res-123", {"cluster_name": "updated"})
        assert result["cluster_name"] == "test-cluster-updated"
    ...
    def test_update_idempotent_no_change(self, mock_api_client, existing_resource):
        ...
        desired = {"cluster_name": existing_resource["cluster_name"]}
        current = {"cluster_name": existing_resource["cluster_name"]}
...
class TestListCluster:
    ...
    def test_list_returns_all(self, mock_api_client):
        ...
        mock_api_client.list.return_value = [
            {"cluster_id": "1", "cluster_name": "first"},
            {"cluster_id": "2", "cluster_name": "second"},
        ]
    ...
    def test_list_with_filter(self, mock_api_client):
        ...
        mock_api_client.list.return_value = [{"cluster_id": "1", "cluster_name": "match"}]
        result = mock_api_client.list("cluster", filters={"cluster_name": "match"})
        assert len(result) == 1
...
class TestIdempotencyCluster:
    ...
    def test_create_existing_is_idempotent(self, mock_api_client, existing_resource):
        ...
        desired_params = {"cluster_name": current["cluster_name"]}
        # If resource exists and matches desired state, no change
        changed = desired_params["cluster_name"] != current["cluster_name"]
        assert changed is False
