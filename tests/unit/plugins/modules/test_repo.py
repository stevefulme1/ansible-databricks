"""Unit tests for stevefulme1.databricks.repo module."""

from unittest.mock import MagicMock

import pytest

MODULE_PATH = "ansible_collections.stevefulme1.databricks.plugins.modules.repo"
CLIENT_PATH = "ansible_collections.stevefulme1.databricks.plugins.module_utils.databricks_client"


@pytest.fixture
def mock_api_client():
    """Mock API client for repo."""
    client = MagicMock()
    client.get.return_value = None
    client.create.return_value = {"repo_id": "res-123", "path": "test-repo"}
    client.update.return_value = {"repo_id": "res-123", "path": "test-repo-updated"}
...
def existing_resource():
    """Return a dict representing an existing repo."""
    return {
        "repo_id": "res-123",
        "path": "test-repo",
        "state": "active",
...
class TestCreateRepo:
    """Tests for creating a repo."""

    def test_create_returns_resource(self, mock_api_client):
        """Verify create returns resource dict with expected fields."""
        result = mock_api_client.create("repo", {"path": "test-repo"})
        assert result["repo_id"] == "res-123"
    ...
    def test_create_with_all_params(self, mock_api_client):
        ...
        mock_api_client.create("repo", params)
        mock_api_client.create.assert_called_once_with("repo", params)
...
class TestUpdateRepo:
    ...
    def test_update_existing_resource(self, mock_api_client, existing_resource):
        ...
        result = mock_api_client.update("repo", "res-123", {"path": "updated"})
        assert result["path"] == "test-repo-updated"
    ...
    def test_update_idempotent_no_change(self, mock_api_client, existing_resource):
        ...
        desired = {"path": existing_resource["path"]}
        current = {"path": existing_resource["path"]}
    ...
    def test_update_nonexistent_raises(self, mock_api_client):
        ...
        with pytest.raises(Exception, match="404 Not Found"):
            mock_api_client.update("repo", "bad-id", {})


class TestDeleteRepo:
    """Tests for deleting a repo."""

    def test_delete_existing(self, mock_api_client, existing_resource):
        """Verify delete calls API with correct ID."""
        mock_api_client.get.return_value = existing_resource
        mock_api_client.delete("repo", "res-123")
        mock_api_client.delete.assert_called_once_with("repo", "res-123")

    def test_delete_nonexistent_is_noop(self, mock_api_client):
        """Verify deleting absent resource reports no change."""
        mock_api_client.get.return_value = None
        result = mock_api_client.get("repo", "missing-id")
...
class TestGetRepo:
    ...
    def test_get_api_timeout(self):
        ...
        with pytest.raises(TimeoutError):
            client.get("repo", "res-123")


class TestListRepo:
    """Tests for listing repo resources."""

    def test_list_returns_all(self, mock_api_client):
        """Verify list returns all resources."""
        mock_api_client.list.return_value = [
            {"repo_id": "1", "path": "first"},
            {"repo_id": "2", "path": "second"},
        ]
        result = mock_api_client.list("repo")
        assert len(result) == 2
    ...
    def test_list_with_filter(self, mock_api_client):
        ...
        mock_api_client.list.return_value = [{"repo_id": "1", "path": "match"}]
        result = mock_api_client.list("repo", filters={"path": "match"})
        assert len(result) == 1


class TestIdempotencyRepo:
    """Tests for idempotent behavior of repo."""
    ...
    def test_create_existing_is_idempotent(self, mock_api_client, existing_resource):
        ...
        mock_api_client.get.return_value = existing_resource
        current = mock_api_client.get("repo", "res-123")
        desired_params = {"path": current["path"]}
        # If resource exists and matches desired state, no change
        changed = desired_params["path"] != current["path"]
        assert changed is False

    def test_delete_absent_is_idempotent(self, mock_api_client):
        """Verify deleting an absent resource reports no change."""
        mock_api_client.get.return_value = None
        exists = mock_api_client.get("repo", "missing") is not None
        assert exists is False


class TestErrorHandlingRepo:
    """Tests for error handling in repo."""
